...
2.  Speicher das Ergebnis in einem neuen Dataframe `lernende2022` mittels des Zuweisungsoperators `<-`.
...
1.  Gruppiere `lernende2022` nach `Stufe` und `Staatsangehoerigkeit` und berechne die Summe der Lernenden pro Gruppe (Häufigkeitstabelle).

2.  Berechne die Prozentwerte der Lernenden pro Gruppe.

3.  Speicher das Ergebnis in einem neuen Daten-Objekt `lernende2022_stufe_staat_sum` mittels des Zuweisungsoperators `<-`.
...
1.  Versuche den Plot aus der Beschreibung für die Zusatzaufgabe mittels des Daten-Objekts `lernende2022_stufe_staat_sum` zu rekonstruieren.
...
```{r}
lernende2022_stufe_traegerschaft_sum <- lernende2022 |> 
...
```{r}
ggplot(data = lernende2022_stufe_traegerschaft_sum,
       mapping = aes(x = Stufe, 
                     y = Prozent, 
                     fill = Traegerschaft)) +
    coord_flip() +
    ...
    geom_text(aes(label = paste0(round(Prozent, 0), "%")),  
              position = position_stack(vjust = 0.5)) +
    labs(title = "Lernende im Kanton Zürich ",
         subtitle = "nach Traegerschaft und Stufe im Jahr 2022",
         fill = "Traegerschaft",
         caption = "Daten: zh.ch/daten",
         y = NULL,
         x = NULL) +
...
lernende2022_stufe_staat_sum_fct <-  lernende2022_stufe_staat_sum |> 
...
ggplot(data = lernende2022_stufe_staat_sum_fct,
       mapping = aes(x = Stufe, 
                     y = Prozent, 
                     fill = Staatsangehoerigkeit)) +
    coord_flip() +
